...
//			ui.click(p);
		...
		LabeledTextLocator artifactPackage = new LabeledTextLocator(
				"Artifact Package:");
		assertEquals("Default package for new " + myType
				+ " Wizard is not that set for the project",
				TestingConstants.DEFAULT_ARTIFACT_PACKAGE, artifactPackage
						.getText(ui));
		...
		CTabItemLocator artifactEditor = new CTabItemLocator(thisArtifactName);
		...
		assertEquals("Project for " + myType + " is incorrect",
				TestingConstants.NEW_MODEL_PROJECT_NAME, project.getText(ui));

		LabeledTextLocator fqn = new LabeledTextLocator("Qualified Name: ");
		assertEquals("FQN for " + myType + " is incorrect",
				TestingConstants.DEFAULT_ARTIFACT_PACKAGE + "."
						+ thisArtifactName, fqn.getText(ui));

		LabeledTextLocator extend = new LabeledTextLocator("Extends: ");
		if (myType.equals("Exception")) {
			assertEquals(
					"Extends for " + myType + " should be fixed on create",
					"java.lang.Exception", extend.getText(ui));
		} else if (myType.equals("Package")) {
			// Packages have no extends!
		} else {
			assertEquals(
					"Extends for " + myType + " should be empty on create", "",
					extend.getText(ui));
		}

		LabeledTextLocator description = new LabeledTextLocator("Description: ");
		assertEquals(
				"Description for " + myType + " should be empty on create", "",
				description.getText(ui));
		...
		// collapse the Section which is open by default sections - for
		// consistent use in the helper
		if (hasEnds) {
			SWTWidgetLocator detailsSection = new SWTWidgetLocator(Label.class,
					"End &Details");
			...
			SWTWidgetLocator literalsSection = new SWTWidgetLocator(
					Label.class, "Constants");
			ui.click(literalsSection);
		} else if (hasMethods) {
			SWTWidgetLocator methodsSection = new SWTWidgetLocator(Label.class,
					"Methods");
		...
		if (hasEnds
				&& (myType.equals("Association") || myType
						.equals("Association Class")))
			items.addAll(ArtifactHelper.associationEndNames(ui,
					thisArtifactName));
		if (hasEnds && myType.equals("Dependency"))
			items.addAll(ArtifactHelper
		...
		if (hasMethods)
			items.add(ArtifactHelper.newMethod(ui, thisArtifactName, prefix
					+ TestingConstants.METHOD_NAMES[0]));

		GuiUtils.maxminTab(ui, thisArtifactName);
		ArtifactHelper.checkArtifactInExplorer(ui,
				TestingConstants.NEW_MODEL_PROJECT_NAME,
