...
public class ProjectPatternTest extends TestCase {
	...
	public void testBuiltinProjectPattern() throws Exception {
		...
		// Check it out!
		assertNotNull(builtinProject);
		assertTrue("Name doesn't match ", builtinProject.getProjectDetails().getName().equals(PROJECTNAME));
	...
	public void testContributedProjectPattern() throws Exception {
		...
		contributedProject = projectPattern.createProject(CONTRIBUTEDPROJECTNAME, null, null);
		...
		// Check it out!
		assertNotNull(contributedProject);
		assertTrue("Name doesn't match ", contributedProject.getProjectDetails().getName().equals(CONTRIBUTEDPROJECTNAME));
		...
		List<Annotation> test1Annos = helper.getAnnotations(contributedProject,TestAnnot1.class);
		
		assertEquals("Wrong number of TestAnnot1 on project", 1, test1Annos.size());
		
		for (Annotation ann : test1Annos){
			if (ann.getContent() instanceof TestAnnot1){
				TestAnnot1 ta1 = (TestAnnot1) ann.getContent();
				assertEquals("TestAnnot1 value on project did not match expected value",ta1.getTwine(),"hemp");
			} else {
