...
public class PatternTest extends TestCase {
	...
	protected void setUp() throws Exception {
		...
		IStereotypeScopeDetails scope = stereotypeOne
		...
		scope.setArtifactLevelTypes(new String[] {
		...
		IStereotypeScopeDetails scopeTwo = stereotypeOne
				.getStereotypeScopeDetails();
		scopeTwo
				.setArtifactLevelTypes(new String[] { IManagedEntityArtifact.class
						.getName() });
		IStereotypeAttribute attribute = StereotypeAttributeFactory
	...
	public void testEnumerationArtifactPattern() throws Exception {
		...
		testEnumPatternArtifact(patternname, targetPackage, entityName,
				"String");
	...
	public void testAssociationArtifactPattern() throws Exception {
		...
		testRelationPatternArtifact(patternname, targetPackage, entityName,
				aEndType, zEndType);
	...
	private IAbstractArtifact testNodePatternArtifact(String patternname,
			String targetPackage, String entityName) throws Exception {
		...
		IArtifactPatternResult artifact = mePattern.createArtifact(project,
				targetPackage, entityName, "");
		...
		assertNotNull("Failed to retrieve artifact for Mgr Session", created);
		assertTrue("Name doesn't match ", created.getName().equals(entityName));
		assertTrue("FQN doesn't match " + fqn, created.getFullyQualifiedName()
				.equals(fqn));
	...
	private IAbstractArtifact testEnumPatternArtifact(String patternname,
			String targetPackage, String entityName, String baseType)
			throws Exception {
		IPattern pattern = PatternFactory.getInstance().getPattern(patternname);
		assertNotNull("No patterns with name '" + patternname + "' returned",
				pattern);
		...
		IArtifactPatternResult artifact = mePattern.createArtifact(project,
				targetPackage, entityName, "", baseType);
		mePattern.addToManager(project, artifact.getArtifact());
		mePattern.annotateArtifact(project, artifact);
		...
		assertEquals("Enum Base Type does not match", "String", enumArtifact
				.getBaseTypeStr());
	...
	private void testRelationPatternArtifact(String patternname,
			String targetPackage, String entityName, String aEndType,
			String zEndType) throws Exception {
		IPattern pattern = PatternFactory.getInstance().getPattern(patternname);
		assertNotNull("No patterns with name '" + patternname + "' returned",
				pattern);
		...
		IArtifactPatternResult artifact = mePattern.createArtifact(project,
				targetPackage, entityName, "", aEndType, zEndType);
		mePattern.addToManager(project, artifact.getArtifact());
		mePattern.annotateArtifact(project, artifact);
	...
	public void testDisabledPattern() throws Exception {
		...
		IPattern pattern = PatternFactory.getInstance().getPattern(patternname);
		assertNull("Patterns with name '" + patternname + "'was returned",
				pattern);
	...
	public void testIRDEntityPattern() throws Exception {
		...
		assertEquals("Wrong number of Stereotypes on artifact", 2, created
				.getStereotypeInstances().size());
		assertNotNull(GENERAL_STEREOTYPE
				+ " stereotype not present on artifact", created
				.getStereotypeInstanceByName(GENERAL_STEREOTYPE));
		IStereotypeInstance attributedSterotype = created
				.getStereotypeInstanceByName(ATTRIBUTED_STEREOTYPE);
		assertNotNull(ATTRIBUTED_STEREOTYPE
				+ " stereotype not present on artifact", attributedSterotype);
		if (attributedSterotype != null) {
			String[] values = attributedSterotype
					.getAttributeValues(STEREOTYPE_ATTRIBUTE_NAME);
			if (values.length != 1) {
				assertTrue(values[0].equals("firstOne"));
			} else {
				assertEquals(ATTRIBUTED_STEREOTYPE
						+ " Wrong number of attributeValues", 1, values.length);
			}
		}

		assertEquals("Wrong number of Annotations on artifact", 2, created
				.getAnnotations().size());
		List<Object> test2Annos = created.getAnnotations(TestAnnot2.class);
		assertEquals("Wrong number of TestAnnot2 on artifact", 1, test2Annos
				.size());
		List<Object> test1Annos = created.getAnnotations(TestAnnot1.class);
		assertEquals("Wrong number of TestAnnot1 on artifact", 1, test2Annos
				.size());
		for (Object ann : test1Annos) {
			if (ann instanceof TestAnnot1) {
				TestAnnot1 ta1 = (TestAnnot1) ann;
				assertEquals(
						"TestAnnot1 value on artifact did not match expected value",
						ta1.getTwine(), "old rope");
			} else {
		...
		for (IField field : created.getFields()) {
			assertTrue("Wrong Field name", field.getName()
					.equals(attributeName));
			assertEquals("Wrong number of Stereotypes on field", 1, field
					.getStereotypeInstances().size());
			assertNotNull(GENERAL_STEREOTYPE
					+ " stereotype not present on field", field
					.getStereotypeInstanceByName(GENERAL_STEREOTYPE));
			assertEquals("Wrong number of Annotations on field", 1, field
					.getAnnotations().size());
			List<Object> test3Annos = field.getAnnotations(TestAnnot3.class);
			assertEquals("Wrong number of TestAnnot3 on field", 1, test3Annos
					.size());
			for (Object ann : test3Annos) {
				if (ann instanceof TestAnnot3) {
					TestAnnot3 ta3 = (TestAnnot3) ann;
					assertEquals(
							"TestAnnot3 value on field did not match expected value",
							ta3.getN(), 666);
				} else {
		...
		assertTrue("Wrong number of Literals",
				created.getLiterals().size() == 1);
		for (ILiteral literal : created.getLiterals()) {
			assertTrue("Wrong Literal name", literal.getName().equals(
					literalName));
			assertEquals("Wrong number of Stereotypes on literal", 1, literal
					.getStereotypeInstances().size());
			assertNotNull(GENERAL_STEREOTYPE
					+ " stereotype not present on literal", literal
					.getStereotypeInstanceByName(GENERAL_STEREOTYPE));
			List<Object> test3Annos = literal.getAnnotations(TestAnnot3.class);
			assertEquals("Wrong number of TestAnnot3 on literal", 1, test3Annos
					.size());
			for (Object ann : test3Annos) {
				if (ann instanceof TestAnnot3) {
					TestAnnot3 ta3 = (TestAnnot3) ann;
					assertEquals(
							"TestAnnot3 value on literal did not match expected value",
							ta3.getN(), 333);
				} else {
					fail("Object is not a TestAnnot3");
				}
		...
		for (IMethod method : created.getMethods()) {
			...
			assertEquals("Wrong number of Stereotypes on method", 1, method
					.getStereotypeInstances().size());
			assertNotNull(GENERAL_STEREOTYPE
					+ " stereotype not present on method", method
					.getStereotypeInstanceByName(GENERAL_STEREOTYPE));
			assertEquals("Wrong number of *return* Stereotypes on method", 1,
					method.getReturnStereotypeInstances().size());
			assertNotNull(GENERAL_STEREOTYPE
					+ " *return* stereotype not present on method", method
					.getReturnStereotypeInstanceByName(GENERAL_STEREOTYPE));
			List<Object> test3Annos = method.getAnnotations(TestAnnot3.class);
			assertEquals("Wrong number of TestAnnot3 on method", 1, test3Annos
					.size());
			for (Object ann : test3Annos) {
				if (ann instanceof TestAnnot3) {
					TestAnnot3 ta3 = (TestAnnot3) ann;
					assertEquals(
							"TestAnnot3 value on method did not match expected value",
							ta3.getN(), 999);
				} else {
					fail("Object is not a TestAnnot3");
				}
			...
			assertEquals("Wrong number of arguments", 1, method.getArguments()
					.size());
			for (IArgument arg : method.getArguments()) {
				assertTrue("Wrong argument name", arg.getName().equals(
						argumentName));
				assertEquals("Wrong number of Stereotypes on argument", 1, arg
						.getStereotypeInstances().size());
				assertNotNull(GENERAL_STEREOTYPE
						+ " stereotype not present on argument", arg
						.getStereotypeInstanceByName(GENERAL_STEREOTYPE));
			}
			assertEquals("Wrong number of exceptions", 1, method
					.getExceptions().size());
			for (IException ex : method.getExceptions()) {
				assertTrue("Wrong exception name", ex.getFullyQualifiedName()
		...
		//
		// String newX = x.replace("old rope", "lots of loot").replace("666",
		// "911");
		//
	...
	public void testIRDAssocPattern() throws Exception {
		...
		// relPattern.executeRequests(project, targetPackage, entityName,
		// extendedArtifact,
		// aEndType,zEndType,true);

		IArtifactPatternResult artifact = relPattern
				.createArtifact(project, targetPackage, entityName,
						extendedArtifact, aEndType, zEndType);
		...
		assertEquals("Extended Artifact does not match", extendedArtifact,
				created.getExtendedArtifact().getFullyQualifiedName());

		assertTrue("Created artrifact is not an Association : "
				+ created.getClass().getName(),
				created instanceof IAssociationArtifact);
		if (created instanceof IAssociationArtifact) {
			...
			if (association.getAEnd() == null) {
				...
				assertEquals("Wrong number of Stereotypes on aEnd", 1,
						association.getAEnd().getStereotypeInstances().size());
				assertNotNull(GENERAL_STEREOTYPE
						+ " stereotype not present on aEnd", association
						.getAEnd().getStereotypeInstanceByName(
								GENERAL_STEREOTYPE));

				List<Object> test3Annos = association.getAEnd().getAnnotations(
						TestAnnot3.class);
				assertEquals("Wrong number of TestAnnot3 on aEnd", 1,
						test3Annos.size());
				for (Object ann : test3Annos) {
					if (ann instanceof TestAnnot3) {
						TestAnnot3 ta3 = (TestAnnot3) ann;
						assertEquals(
								"TestAnnot3 value on aEnd did not match expected value",
								ta3.getN(), 111);
					} else {
						fail("Object is not a TestAnnot3");
					}
			...
			if (association.getZEnd() == null) {
				...
				assertEquals("Wrong number of Stereotypes on zEnd", 1,
						association.getZEnd().getStereotypeInstances().size());
				assertNotNull(ATTRIBUTED_STEREOTYPE
						+ " stereotype not present on zEnd", association
						.getZEnd().getStereotypeInstanceByName(
								ATTRIBUTED_STEREOTYPE));
			}
			assertEquals("AEndType does not match", aEndType, association
					.getAEnd().getType().getFullyQualifiedName());
			assertEquals("ZEndType does not match", zEndType, association
					.getZEnd().getType().getFullyQualifiedName());

			List<Object> test3Annos = association.getZEnd().getAnnotations(
					TestAnnot3.class);
			assertEquals("Wrong number of TestAnnot3 on zEnd", 1, test3Annos
					.size());
			for (Object ann : test3Annos) {
				if (ann instanceof TestAnnot3) {
					TestAnnot3 ta3 = (TestAnnot3) ann;
					assertEquals(
							"TestAnnot3 value on zEnd did not match expected value",
							ta3.getN(), 222);
				} else {
					fail("Object is not a TestAnnot3");
				}
