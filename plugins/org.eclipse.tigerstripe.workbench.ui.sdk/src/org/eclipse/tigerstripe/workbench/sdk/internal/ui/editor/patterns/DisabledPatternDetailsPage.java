...
public class DisabledPatternDetailsPage implements IDetailsPage {


	/**
	 * An adapter that will listen for changes on the form
	 */
	private class DisabledPatternDetailsPageListener implements ModifyListener,
	...
	private DisabledPatternSection master;

	private DisabledPatternContribution contribution;

	private boolean silentUpdate = false;

	private Text disabledPatternText;

	private Text contributorText;

	public DisabledPatternDetailsPage() {
		super();
	...
	// ============================================================
	private void setContribution(DisabledPatternContribution contribution) {
		this.contribution = contribution;
	}

	private DisabledPatternContribution getContribution() {
		return contribution;
	}
	...
	private void createContributionInfo(Composite parent) {
		...
		DisabledPatternDetailsPageListener adapter = new DisabledPatternDetailsPageListener();
		...
		disabledPatternText = toolkit.createText(sectionClient, "");

		disabledPatternText.setLayoutData(new GridData(GridData.FILL_HORIZONTAL));
		disabledPatternText.addModifyListener(adapter);
...
//		label.setEnabled(!isReadOnly);
	...
	public void selectionChanged(IFormPart part, ISelection selection) {
		if (part instanceof DisabledPatternSection) {
			//if (nameEditListener != null)
			//	nameEditListener.reset();

			master = (DisabledPatternSection) part;
			Table fieldsTable = master.getViewer().getTable();

			DisabledPatternContribution selected = (DisabledPatternContribution) fieldsTable.getSelection()[0].getData();
