...
public class ArtifactMetadataDetailsPage extends CommonDetailsPage implements IDetailsPage {
	 ...
	 */
	private class ArtifactMetadataDetailsPageListener implements ModifyListener,
	...
	private ArtifactMetadataSection master;

	private ArtifactMetadataContribution contribution;
	...
	public ArtifactMetadataDetailsPage() {
		super();
	...
	// ============================================================
	private void setContribution(ArtifactMetadataContribution contribution) {
		this.contribution = contribution;
	}

	private ArtifactMetadataContribution getContribution() {
		return contribution;
	}
	...
	private void createContributionInfo(Composite parent) {
		...
		ArtifactMetadataDetailsPageListener adapter = new ArtifactMetadataDetailsPageListener();
...
//		}
		...
		label = toolkit.createLabel(sectionClient, "User Label: ");
//		label.setEnabled(!isReadOnly);
		userLabelText = toolkit.createText(sectionClient, "");
		...
		fieldsButton = toolkit.createButton(opComposite, "Has Fileds", SWT.CHECK);
		methodsButton = toolkit.createButton(opComposite, "Has Methods", SWT.CHECK);
		literalsButton = toolkit.createButton(opComposite, "Has Literals", SWT.CHECK);
		label = toolkit.createLabel(sectionClient, "");
...
//		label.setEnabled(!isReadOnly);
	...
	public void selectionChanged(IFormPart part, ISelection selection) {
		if (part instanceof ArtifactMetadataSection) {
			...
			master = (ArtifactMetadataSection) part;
			Table fieldsTable = master.getViewer().getTable();

			ArtifactMetadataContribution selected = (ArtifactMetadataContribution) fieldsTable.getSelection()[0].getData();
	...
	private void updateForm() {
		...
		userLabelText.setText(getContribution().getUserLabel());
		userLabelText.setEnabled(!getContribution().isReadOnly());
		
		fieldsButton.setSelection(getContribution().isHasFields());
		fieldsButton.setEnabled(!getContribution().isReadOnly());
		
		methodsButton.setSelection(getContribution().isHasMethods());
		methodsButton.setEnabled(!getContribution().isReadOnly());
		
		literalsButton.setSelection(getContribution().isHasLiterals());
		literalsButton.setEnabled(!getContribution().isReadOnly());
