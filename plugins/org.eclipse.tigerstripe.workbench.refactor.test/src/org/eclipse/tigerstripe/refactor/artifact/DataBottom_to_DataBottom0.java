...
public class DataBottom_to_DataBottom0 extends UITestCaseSWT {
	...
	public static void internalCheckDiagram(IUIContext ui,
			LocatorHelper helper, String artifactPrefix, String artifactName) {

		// Basic rename
		try {
			ui.click(helper.getDatatypeLocator(ui, artifactPrefix
					+ artifactName));
		} catch (Exception e) {
			fail("Refactored Entity '" + artifactPrefix + artifactName
					+ "' not found on diagram");
		}

		IWidgetLocator attr = helper.getDatatypeAttributeLocator(ui,
				artifactPrefix + artifactName, "recur");
		WidgetReference attrRef = (WidgetReference) attr;
		Attribute5EditPart.AttributeLabelFigure fig = (Attribute5EditPart.AttributeLabelFigure) attrRef
				.getWidget();
	 ...
	 * 
	 * We need to chck the thing we just renamed - which gets closed when we do
	 * a rename
	 * 
	 */
	public static void checkEditorUpdated(IUIContext ui) throws Exception {
		...
		ui.click(2, new TreeItemLocator(project + "/src/simple/DataBottom0",
				view));
		...
		assertEquals("Referenced type not updated in Editor",
				"simple.DataBottom0", type.getText(ui));
