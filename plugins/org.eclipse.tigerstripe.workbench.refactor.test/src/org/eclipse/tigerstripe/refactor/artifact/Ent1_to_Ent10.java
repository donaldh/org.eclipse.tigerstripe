...
public class Ent1_to_Ent10 extends UITestCaseSWT {
	...
	public static void checkDiagramsAfterMove(IUIContext ui) throws Exception{
		...
		internalCheckDiagram(ui, helper, artifactPrefix, myArtifactPrefix, "Ent1");
	...
	public static void checkDiagrams(IUIContext ui) throws Exception{
		...
		ui.click(new CTabItemLocator("default.wvd"));
		String artifactPrefix = "";
		internalCheckDiagram(ui, helper, artifactPrefix, artifactPrefix, "Ent10");
		artifactPrefix = "simple.";
		ui.click(new CTabItemLocator("inside-moved.wvd"));
		internalCheckDiagram(ui, helper, artifactPrefix, artifactPrefix,"Ent10");
		ui.click(new CTabItemLocator("outside-class-diagram.wvd"));
	...
	public static void internalCheckDiagram(IUIContext ui, LocatorHelper helper, String artifactPrefix, String myArtifactPrefix, String artifactName) throws Exception {
		
		// Basic rename
		try {
			ui.click(helper.getManagedEntityLocator(ui, myArtifactPrefix+artifactName));
		} catch (Exception e){
		...
		String expectedText = "+method0("+artifactName+"):"+artifactName+"[0..1]";
...
//				"simple","Dependency0",items);
	...
	public static void checkEditorUpdated(IUIContext ui, String myPackage, String name) throws Exception{
		...
		assertEquals("Association End type not updated in Editor","ent1_0"+":"+name, endName);
		...
		assertEquals("Association End type not updated in Editor","ent1_2"+":"+name, endName);
		...
		endName = endNames.get(0);
		assertEquals("Association End type not updated in Editor","ent1_5"+":"+name, endName);
		...
		endName = endNames.get(0);
		assertEquals("Association Class End type not updated in Editor","ent1_3"+":"+name, endName);
		endName = endNames.get(1);
		assertEquals("Association Class End type not updated in Editor","ent1_4"+":"+name, endName);
		
		ui.click(new CTabItemLocator("*Ent2"));
		// Check for Extends in this one
		LabeledTextLocator extend = new LabeledTextLocator("Extends: ");
		assertEquals("Extended type not updated in Editor",newName,extend.getText(ui));
		// Now check Method
		...
		TableItemLocator attributeNameInTable = new TableItemLocator("method0("+name+"):"+name+"[0..1]");
		...
		assertEquals("Referenced type (Method return) not updated in Editor",newName,type.getText(ui));
		try {
...
//		assertEquals("Query Return type not updated in Editor","Ent10", retType.getText(ui));
	...
	public static void checkAPI(String myPackage, String name) throws Exception{
		...
		assertNotNull("New artifact '"+newName+"' is still not being returned from the Art Mgr", ent10);
