...
public class ArtifactNameValidator {
	...
	public static IStatus validateArtifactName(String artifactName) {
		...
		IStatus validatedStatus = validateConventionsStatus(JavaConventions
				.validateJavaTypeName(artifactName));
	...
	public static IStatus validatePackageArtifactName(String artifactName) {
		...
		IStatus validatedConventionStatus = validateConventionsStatus(JavaConventions
				.validatePackageName(artifactName));
	...
	public static IStatus validateArtifactDoesNotExist(
		...
		if (project.getArtifactManagerSession()
				.getArtifactByFullyQualifiedName(fullyQualifiedName, false) != null) {
			status
	...
	public static IStatus validatePackageArtifactDoesNotExist(
			IJavaProject project, String artifactPackageName) {

		StatusInfo status = new StatusInfo();

		if (artifactPackageName != null && project != null) {

			IPath path = project.getPath().append(
					"src" + '/' + artifactPackageName.replaceAll("\\.", "/"));
			try {
	...
	private static IStatus validateConventionsStatus(IStatus status) {
		...
		if (status.getSeverity() == IStatus.ERROR) {
			statusInfo
					.setError(NewWizardMessages
							.getFormattedString(
									"NewArtifactWizardPage.error.InvalidArtifactName", status.getMessage())); //$NON-NLS-1$
		} else if (status.getSeverity() == IStatus.WARNING) {
			statusInfo
					.setWarning(NewWizardMessages
							.getFormattedString(
									"NewArtifactWizardPage.warning.TypeNameDiscouraged", status.getMessage())); //$NON-NLS-1$
		}
