 ...
 */
public class UmlDatatypeMappingEditDialog extends TSMessageDialog {

	private UmlDatatypeMapping mapping;

	private StringDialogField umlDatatypeField;
	...
	public UmlDatatypeMappingEditDialog(Shell parentShell,
			UmlDatatypeMapping mapping, List existingMappings) {
		super(parentShell);
		...
		umlDatatypeField = new StringDialogField();
		umlDatatypeField.setDialogFieldListener(adapter);
		umlDatatypeField.setLabelText("UML Datatype:"); //$NON-NLS-1$
	...
	protected void setDefaultMessage() {
		setMessage("Specify UMLDatatype Mapping Details");
	}
	...
	protected boolean validateMapping() {
		...
		if ("".equals(mapping.getUmlDatatype())) {
			okEnabled = false;
			setErrorMessage("Invalid UML datatype");
			return okEnabled;
		}
		...
		for (Iterator iter = this.existingMappings.iterator(); iter.hasNext();) {
			UmlDatatypeMapping map = (UmlDatatypeMapping) iter.next();
			if (map.getUmlDatatype().equals(mapping.getUmlDatatype())) {
				okEnabled = false;
				setErrorMessage("Duplicate UMLDatatype mapping.");
			}
		}
	...
	public Control createDialogArea(Composite parent) {
		...
		getShell().setText("UMLDatatype Mapping Details");
	...
	protected void initDialog() {
		...
		umlDatatypeField.setText(mapping.getUmlDatatype());
	...
	private void createMappingControls(Composite composite, int nColumns) {
		umlDatatypeField.doFillIntoGrid(composite, nColumns);

		DialogField.createEmptySpace(composite, nColumns);
		mappedDatatypeField.doFillIntoGrid(composite, nColumns);

		LayoutUtil.setWidthHint(umlDatatypeField.getTextControl(null),
				getMaxFieldWidth());
	...
	protected void attributeFieldChanged(DialogField field) {
		if (field == umlDatatypeField) {
			mapping.setUmlDatatype(umlDatatypeField.getText().trim());
