...
/**
 * A set of incoming edges that acts like one edge. Used internally for laying
 * out connected border nodes.
 * 
 * @author aboyko
 * @since 2.1
 */
class JointIncomingEdges extends Edge implements JointEdges {
	...
	public Edge getLeadingEdge() {
		if (leadingEdge == null) {
			for (int i = 0; i < edges.size(); i++) {
				...
				int cost = getBendpointX(e) - e.target.x
	...
	private int getBendpointX(Edge e) {
		Node node = e.vNodes == null ? e.source : e.vNodes.getNode(e.vNodes
				.size() - 1);
