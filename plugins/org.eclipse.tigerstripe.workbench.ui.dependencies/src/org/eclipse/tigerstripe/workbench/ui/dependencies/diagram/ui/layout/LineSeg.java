 ...
 * designated as the origin and the other is the terminal.
 * 
 * @author sshaw
...
public class LineSeg implements Cloneable, java.io.Serializable, Translatable {
	 ...
	 *            direction
	 */
	public LineSeg(final KeyPoint start, final int fromX, final int fromY,
			final float slope, final long len, final int xdir) {
		super();
	...
	public final Point getSupremum() {
		return new Point(Math.max(origin.x, terminus.x), Math.max(origin.y,
				terminus.y));
	}
	...
	public final Point getInfimum() {
		return new Point(Math.min(origin.x, terminus.x), Math.min(origin.y,
				terminus.y));
	}
	...
	public final float slope() {
		...
		return (float) (terminus.y - origin.y)
				/ (float) (terminus.x - origin.x);
	}
	...
	public final boolean containsPoint(final Point aPoint, final int tolerance) {
		/*
		 * We need perform the calculations in double numbers to avoid possible
		 * integer overflows in Point#getDistance2() method
		 */
		double lengthOfSegment = Math.sqrt((origin.x - terminus.x)
				* (origin.x - terminus.x) + (origin.y - terminus.y)
				* (origin.y - terminus.y));
		double lengthFromOriginToPoint = Math.sqrt((origin.x - aPoint.x)
				* (origin.x - aPoint.x) + (origin.y - aPoint.y)
				* (origin.y - aPoint.y));
		double lengthFromTerminusToPoint = Math.sqrt((terminus.x - aPoint.x)
				* (terminus.x - aPoint.x) + (terminus.y - aPoint.y)
				* (terminus.y - aPoint.y));

		return lengthFromTerminusToPoint + lengthFromOriginToPoint
				- lengthOfSegment <= tolerance;
	}
	...
	public final float distanceAlong(Point coord) {
		 ...
		 * t < 1 and d = p2 - p (direction vector)
		 * 
		 * To find out if point lies "inside" line segment (i.e. can draw
		 * perpendicular line from segment to point), use projection of point
		 * (q) to line (p + td): t = (q-p).d/length(d)^2 (. is dot product)
		 */
	...
	public final long distanceToPoint(final int xCoord, final int yCoord) {
		...
		long d1 = Math
				.round(getOrigin().getDistance(new Point(xCoord, yCoord)));
		long d2 = Math.round(getTerminus().getDistance(
	...
	public final Point perpIntersect(final int startX, final int startY) {
		...
		fx = (m * startY - m * getOrigin().y + m * m * getOrigin().x + startX)
				/ (float) (m * m + 1.0);
	 ...
	 * 
	 * B (the point we are looking for) + | dist |h this segment
	 * P1-----------+-------------------> A
	 * 
	 ...
	 */
	public final Point locatePoint(final double pctDist, final long theHeight,
		...
		LineSeg linesegAB = new LineSeg(KeyPoint.ORIGIN, pt.x, pt.y,
				perpSlope(), theHeight, xdir);
	 ...
	 */
	public final boolean pointOn(final long theDistance,
			final KeyPoint fromKeyPoint, Point ptResult) {
		float m, dx_float;
	...
	public TrigValues getTrigValues(final Ray ptToVector) {
		...
		// vector 1 can
		// be calculated
		double dRotateX = (ptToVector.x * dCosAlpha)
				- (ptToVector.y * dSinAlpha);
		double dRotateY = (ptToVector.x * dSinAlpha)
				+ (ptToVector.y * dCosAlpha);
	...
	public final LineSeg getParallelLineSegThroughPoint(Point ptLoc) {
		if (isHorizontal()) {
			return new LineSeg(new Point(getOrigin().x, ptLoc.y), new Point(
					getTerminus().x, ptLoc.y));
		} else if (isVertical()) {
			return new LineSeg(new Point(ptLoc.x, getOrigin().y), new Point(
					ptLoc.x, getTerminus().y));
		} else {
			...
			return new LineSeg(locatePoint(0.0, nHeight, position),
	...
	public PointList getLinesIntersections(LineSeg line) {
		...
		if (det == 0) {
			if (a1 == a2 && b1 == b2 && c1 == c2) {
				...
				// intersections
				// we will put the 2 points out of 4 points - ends of 2
				// segments. Look above.
				intersections.addPoint(points.get(1));
				intersections.addPoint(points.get(2));
			}
		} else {
			intersections.addPoint(new PrecisionPoint(
	...
	public PointList getLineIntersectionsWithEllipse(Rectangle ellipseBounds) {
		...
		PrecisionPoint preciseOrigin = new PrecisionPoint(origin);
		PrecisionPoint preciseTerminus = new PrecisionPoint(terminus);
		PrecisionRectangle preciseEllipseBounds = new PrecisionRectangle(
				ellipseBounds);
		if (preciseEllipseBounds.preciseWidth == 0
				|| preciseEllipseBounds.preciseHeight == 0)
			return intersections;
		...
		if (b == 0) {
			...
			if (y < 0)
				return intersections;
			intersections.addPoint(new PrecisionPoint(x
					+ ellipsePreciseCenter.preciseX, Math.sqrt(y)
					+ ellipsePreciseCenter.preciseY));
			intersections.addPoint(new PrecisionPoint(x
					+ ellipsePreciseCenter.preciseX, -Math.sqrt(y)
					+ ellipsePreciseCenter.preciseY));
		} else {
			...
			intersections.addPoint(new PrecisionPoint(x1
					+ ellipsePreciseCenter.preciseX, (c - a * x1) / b
					+ ellipsePreciseCenter.preciseY));
			intersections.addPoint(new PrecisionPoint(x2
					+ ellipsePreciseCenter.preciseX, (c - a * x2) / b
					+ ellipsePreciseCenter.preciseY));
		}
	...
	public PointList getLineIntersectionsWithLineSegs(final PointList points) {
		PointList intersections = new PrecisionPointList();
		if (points.size() < 2) {
			if (containsPoint(points.getFirstPoint(),
					DEFAULT_INTERSECTION_TOLERANCE)) {
				intersections.addPoint(points.getFirstPoint());
			}
		} else {
			for (int i = 0; i < points.size() - 1; i++) {
				LineSeg seg = new LineSeg(points.getPoint(i),
						points.getPoint(i + 1));
				...
				for (int j = 0; j < currentIntersections.size(); j++) {
					...
					if (seg.containsPoint(intersection,
							DEFAULT_INTERSECTION_TOLERANCE)) {
						intersections
								.addPoint(currentIntersections.getPoint(j));
					}
				}
			}
		}
