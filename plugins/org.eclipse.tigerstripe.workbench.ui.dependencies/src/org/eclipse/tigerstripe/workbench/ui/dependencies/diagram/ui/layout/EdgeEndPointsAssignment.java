...
class EdgeEndPointsAssignment {
	...
	private void assignEndPointsForEdgesFromNode(Node node) {
		...
		if (node instanceof ConstantSizeNode) {
			initEdgesSets((ConstantSizeNode) node, incoming, outgoing,
					specialBorderNodes);
		} else {
		...
		if (node instanceof ConstantSizeNode
				&& ((ConstantSizeNode) node).minIncomingPadding > 0) {
			assignEdgesEndPoints(incoming, (ConstantSizeNode) node, true);
		} else {
			for (int i = 0; i < incoming.size(); i++) {
				setEndPoint(incoming.getEdge(i),
						new Point(node.x + node.getOffsetIncoming(), node.y));
			}
		}

		if (node instanceof ConstantSizeNode
				&& ((ConstantSizeNode) node).minOutgoingPadding > 0) {
			assignEdgesEndPoints(outgoing, (ConstantSizeNode) node, false);
		} else {
			for (int i = 0; i < outgoing.size(); i++) {
				setStartPoint(outgoing.getEdge(i),
						new Point(node.x + node.getOffsetOutgoing(), node.y
								+ node.height));
			}
		}

		if (node instanceof ConstantSizeNode) {
			assignEndPointsForJointEdgeWithIncomingAndOutgoingEdges(
					(ConstantSizeNode) node, specialBorderNodes);
		}
	}

	private void initEdgesSets(ConstantSizeNode n, EdgeList incoming,
			EdgeList outgoing, List<BorderNode> specialBorderNodes) {
		...
		for (Iterator<BorderNode> itr = n.borderNodes.iterator(); itr.hasNext();) {
			...
			if (!(borderNode.incomingJointEdges.edges.isEmpty() ^ borderNode.outgoingJointEdges.edges
					.isEmpty())) {
		...
		Collections.sort(incoming, new Comparator<Edge>() {
			public int compare(Edge e1, Edge e2) {
				return getIncomingEdgeBendpointX(e1, graph)
						- getIncomingEdgeBendpointX(e2, graph);
			}
		});

		Collections.sort(outgoing, new Comparator<Edge>() {
			public int compare(Edge e1, Edge e2) {
				return getOutogingEdgeBendpointX(e1, graph)
						- getOutogingEdgeBendpointX(e2, graph);
			}
		});
	}

	private void assignEdgesEndPoints(EdgeList edges, ConstantSizeNode n,
			boolean end) {
		int leftIndex;
		int rightIndex;
		...
		for (leftIndex = 0; leftIndex < edges.size(); leftIndex++) {
			...
			leftBendpointX = end ? getIncomingEdgeBendpointX(e, graph)
					: getOutogingEdgeBendpointX(e, graph);
			if (leftX < leftBendpointX
					&& leftBendpointX < rightX
					&& (float) (leftBendpointX - leftX) / (leftIndex + 1) >= padding) {
				break;
			}
		}
		for (rightIndex = edges.size() - 1; rightIndex >= leftIndex; rightIndex--) {
			Edge e = edges.getEdge(rightIndex);
			rightBendpointX = end ? getIncomingEdgeBendpointX(e, graph)
					: getOutogingEdgeBendpointX(e, graph);
			if (leftX < rightBendpointX
					&& rightBendpointX < rightX
					&& (float) (rightX - rightBendpointX)
							/ (edges.size() - rightIndex) >= padding) {
				break;
			}
		}
		...
		if (rightIndex >= leftIndex) {
			uniformlyPadEdges(edges, 0, leftIndex, new Point(leftX, y),
					new Point(leftBendpointX, y), end);
			uniformlyPadEdges(edges, rightIndex + 1, edges.size(), new Point(
					rightBendpointX, y), new Point(rightX, y), end);
			makeStraight(edges, leftIndex, rightIndex + 1, end);
		} else {
			uniformlyPadEdges(edges, 0, edges.size(), new Point(leftX, y),
					new Point(rightX, y), end);
		}
	}

	private void uniformlyPadEdges(List edges, int startIndex, int endIndex,
			Point startPoint, Point endPoint, boolean end) {
		...
		for (int i = startIndex; i < endIndex; i++) {
			...
			Point p = startPoint.getCopy().translate(
					diff.getCopy().scale(coefficient));
			if (end) {
				setEndPoint(e, p);
			} else {
				setStartPoint(e, p);
	...
	private void makeStraight(EdgeList edges, int startIndex, int endIndex,
			boolean end) {
		for (int i = startIndex; i < endIndex; i++) {
			...
			if (end) {
				setEndPoint(e,
						new Point(getIncomingEdgeBendpointX(e, graph), y));
			} else {
				setStartPoint(e, new Point(getOutogingEdgeBendpointX(e, graph),
						y));
			}
		}
	}
	...
	private void setStartPoint(Edge e, Point p) {
		if (e instanceof JointEdges) {
			...
			if (p == null) {
				if (e instanceof ConstrainedEdge
						&& ((ConstrainedEdge) e).sourceConstraint != null) {
					p = ((ConstrainedEdge) e).sourceConstraint
							.getEdgesDefaultEndPoint();
				} else {
					p = new Point(e.source.x + e.source.getOffsetOutgoing(),
	...
	private void setEndPoint(Edge e, Point p) {
		if (e instanceof JointEdges) {
			...
			if (p == null) {
				if (e instanceof ConstrainedEdge
						&& ((ConstrainedEdge) e).targetConstraint != null) {
					p = ((ConstrainedEdge) e).targetConstraint
							.getEdgesDefaultEndPoint();
				} else {
					p = new Point(e.target.x + e.target.getOffsetIncoming(),
							e.target.y);
				}
			}
			e.end = p;
	...
	private void assignEndPointsForJointEdgeWithIncomingAndOutgoingEdges(
			ConstantSizeNode node, List<BorderNode> specialBorderNodes) {
		Collections.sort(specialBorderNodes, new Comparator<BorderNode>() {
			public int compare(BorderNode bn1, BorderNode bn2) {
				return bn1.incomingJointEdges.edges.size()
						+ bn1.outgoingJointEdges.edges.size()
						- bn2.incomingJointEdges.edges.size()
						- bn2.outgoingJointEdges.edges.size();
			}
		});

		List<BorderNode> leftSideBorderNodes = new ArrayList<BorderNode>(
				specialBorderNodes.size() / 2 + 1);
		List<BorderNode> rightSideBorderNodes = new ArrayList<BorderNode>(
				specialBorderNodes.size() / 2 + 1);

		for (Iterator<BorderNode> itr = specialBorderNodes.iterator(); itr
				.hasNext();) {
		...
		Collections.sort(leftSideBorderNodes, new Comparator<BorderNode>() {
			public int compare(BorderNode bn1, BorderNode bn2) {
				return bn1.outgoingJointEdges.edges.size()
						- bn1.incomingJointEdges.edges.size()
						- (bn2.outgoingJointEdges.edges.size() - bn2.incomingJointEdges.edges
								.size());
			}
		});
		...
		uniformlyPadBorderNodes(leftSideBorderNodes, 0,
				leftSideBorderNodes.size(), new Point(node.x, node.y),
				new Point(node.x, node.y + node.height));
		uniformlyPadBorderNodes(rightSideBorderNodes, 0,
				rightSideBorderNodes.size(), new Point(node.x + node.width,
						node.y), new Point(node.x + node.width, node.y
						+ node.height));

		for (Iterator<BorderNode> itr = leftSideBorderNodes.iterator(); itr
				.hasNext();) {
			BorderNode bn = itr.next();
			if (!bn.incomingJointEdges.edges.isEmpty()
					|| !bn.outgoingJointEdges.edges.isEmpty()) {
				assignEndPointsForEdgesFromBorderNode(bn);
			}
		}
		for (Iterator<BorderNode> itr = rightSideBorderNodes.iterator(); itr
				.hasNext();) {
			BorderNode bn = itr.next();
			if (!bn.incomingJointEdges.edges.isEmpty()
					|| !bn.outgoingJointEdges.edges.isEmpty()) {
	...
	private void uniformlyPadBorderNodes(List<BorderNode> borderNodes,
			int startIndex, int endIndex, Point startPoint, Point endPoint) {
		Dimension diff = endPoint.getDifference(startPoint);
		int numPieces = endIndex - startIndex + 1;
		for (int i = startIndex; i < endIndex; i++) {
			float coefficient = (float) (i - startIndex + 1) / numPieces;
		...
		if (node.minIncomingPadding > 0 || node.minOutgoingPadding > 0) {
			if (node.position == PositionConstants.NORTH) {
				...
				if (node.position == PositionConstants.WEST) {
					incomingStartPt = new Point(node.x, node.y
							+ (node.incomingJointEdges.edges.size() + 1)
							* node.height
							/ (node.incomingJointEdges.edges.size()
									+ node.outgoingJointEdges.edges.size() + 1));
					incomingEndPt = new Point(node.x, node.y);
					outgoingStartPt = new Point(node.x, node.y
							+ node.incomingJointEdges.edges.size()
							* node.height
							/ (node.incomingJointEdges.edges.size()
									+ node.outgoingJointEdges.edges.size() + 1));
					...
					incomingEndPt = new Point(node.x + node.width, node.y
							+ (node.incomingJointEdges.edges.size() + 1)
							* node.height
							/ (node.incomingJointEdges.edges.size()
									+ node.outgoingJointEdges.edges.size() + 1));
					outgoingStartPt = new Point(node.x + node.width, node.y
							+ node.height);
					outgoingEndPt = new Point(node.x + node.width, node.y
							+ node.incomingJointEdges.edges.size()
							* node.height
							/ (node.incomingJointEdges.edges.size()
									+ node.outgoingJointEdges.edges.size() + 1));
				}
				uniformlyPadEdges(node.incomingJointEdges.edges, 0,
						node.incomingJointEdges.edges.size(), incomingStartPt,
						incomingEndPt, true);
				uniformlyPadEdges(node.outgoingJointEdges.edges, 0,
						node.outgoingJointEdges.edges.size(), outgoingStartPt,
						outgoingEndPt, false);
			}
		} else {
			...
			for (int i = 0; i < node.outgoingJointEdges.edges.size(); i++) {
				setStartPoint(node.outgoingJointEdges.edges.getEdge(i),
						defaultPt);
			}
		}
	}
	...
	static int getOutogingEdgeBendpointX(Edge e, DirectedGraph g) {
		if (e instanceof JointEdges) {
			return getOutogingEdgeBendpointX(((JointEdges) e).getLeadingEdge(),
					g);
		} else {
			if (e.vNodes == null) {
				if (e.end != null) {
					...
					if (e.target instanceof ConstantSizeNode
							&& ((ConstantSizeNode) e.target)
									.getMinIncomingPadding() > 0
							&& e.target.x < sourceX
							&& sourceX < e.target.x + e.target.width) {
						return sourceX;
					} else {
				...
				int targetX = e.end != null ? e.end.x : e.target.x
						+ e.target.getOffsetIncoming();
				if (targetX > rightX) {
					return rightX;
	...
	static int getIncomingEdgeBendpointX(Edge e, DirectedGraph g) {
		if (e instanceof JointEdges) {
			return getIncomingEdgeBendpointX(((JointEdges) e).getLeadingEdge(),
					g);
		} else {
			if (e.vNodes == null) {
				if (e.start != null) {
					...
					if (e.source instanceof ConstantSizeNode
							&& ((ConstantSizeNode) e.source)
									.getMinOutgoingPadding() > 0
							&& e.source.x < targetX
							&& targetX < e.source.x + e.source.width) {
						return targetX;
					} else {
				...
				int sourceX = e.start != null ? e.start.x : e.source.x
						+ e.source.getOffsetOutgoing();
				if (sourceX > rightX) {
					return rightX;
	...
	private static int getLeftX(Node vn, Edge e, DirectedGraph g) {
		return vn.getLeft() != null ? vn.getLeft().x + vn.getLeft().width
				+ g.getPadding(vn.getLeft()).right + e.getPadding() - 1 : 0;
	}

	private static int getRightX(Node vn, Edge e, DirectedGraph g) {
		return vn.getRight() != null ? vn.getRight().x
				- g.getPadding(vn.getRight()).left - e.getPadding() + 1 : g
				.getLayoutSize().width;
	}
