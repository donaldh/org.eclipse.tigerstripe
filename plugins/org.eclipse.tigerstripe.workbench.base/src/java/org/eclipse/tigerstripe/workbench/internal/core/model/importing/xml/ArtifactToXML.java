...
public class ArtifactToXML {
	...
	public Element getRootElement(ITigerstripeModelProject modelProject){
		Element rootElement = document.createElementNS(ArtifactToXML.TS_NSURI, "tigerstripeProject");
		rootElement.setPrefix("ts");
		rootElement.setAttributeNS("http://www.w3.org/2001/XMLSchema-instance", "xsi:schemaLocation", ArtifactToXML.TS_NSURI.concat(" tigerstripeExportSchema-v2-0.xsd"));
		rootElement.setAttribute("name", modelProject.getName());
		try{
		rootElement.setAttribute("version",modelProject.getProjectDetails().getVersion());
		}
	...
	private Element getComment(ITigerstripeModelProject model){
		String description =null;
		try {
			description = model.getProjectDetails().getDescription();
		} catch (TigerstripeException t){
		...
		if (description != null && description.length() > 0){
			...
			commentElement.appendChild(document.createTextNode(escaper.encode(description)));
	...
	public Element artifactToIndexElement(IAbstractArtifact artifact){
		// name, fileName, artifactType
		Element artifactElement = document.createElementNS(TS_NSURI, "artifactFile");
		artifactElement.setPrefix("ts");		
		artifactElement.setAttribute("artifactType", artifact.getArtifactType());
		artifactElement.setAttribute("fileName", artifact.getFullyQualifiedName().replace('.', File.separator.charAt(0)).concat(".xml"));
		artifactElement.setAttribute("name", artifact.getFullyQualifiedName());
		return artifactElement;
	}
	
	public Element artifactToElement (IAbstractArtifact artifact){
		return artifactToElement(artifact, true, true);
	}
	
	// This is what the export XML plugin does!
	public Element artifactToElement (IAbstractArtifact artifact, boolean includeEndNames, boolean useRealName){
