...
public abstract class Rule extends BaseContainerObject implements
	 ...
	 * 
	 * @param pluginConfig
	 * @param includeDependencyOverride
	 *            This implements the logic for "Run all rules as Local" it
	 *            restricts the scope of the "allXXX" collections for 'global'
	 *            down to 'local'
	 * @return
	 * @throws TigerstripeException
	 */
	public Map<String, Object> getGlobalContext(
			PluggablePluginConfig pluginConfig,
			boolean includeDependencyOverride) throws TigerstripeException {
		...
		Collection<IAbstractArtifact> allArtifacts = artifactMgr
				.getAllArtifacts(includeDependencyOverride, false,
		...
		if (artifactMgr.getRegisteredArtifacts().contains(
			...
			Collection<IAbstractArtifact> allEntities = artifactMgr
					.getArtifactsByModel(ManagedEntityArtifact.MODEL,
							includeDependencyOverride, false,
							new NullProgressMonitor());
		...
		if (artifactMgr.getRegisteredArtifacts().contains(
			...
			Collection<IAbstractArtifact> allDatatypes = artifactMgr
					.getArtifactsByModel(DatatypeArtifact.MODEL,
							includeDependencyOverride, false,
							new NullProgressMonitor());
		...
		if (artifactMgr.getRegisteredArtifacts().contains(EventArtifact.MODEL)) {
			...
			Collection<IAbstractArtifact> allEvents = artifactMgr
					.getArtifactsByModel(EventArtifact.MODEL,
							includeDependencyOverride, false,
							new NullProgressMonitor());
		...
		if (artifactMgr.getRegisteredArtifacts().contains(EnumArtifact.MODEL)) {
			...
			Collection<IAbstractArtifact> allEnums = artifactMgr
					.getArtifactsByModel(EnumArtifact.MODEL,
							includeDependencyOverride, false,
							new NullProgressMonitor());
		...
		if (artifactMgr.getRegisteredArtifacts().contains(
			...
			Collection<IAbstractArtifact> allExceptions = artifactMgr
					.getArtifactsByModel(ExceptionArtifact.MODEL,
							includeDependencyOverride, false,
							new NullProgressMonitor());
		...
		if (artifactMgr.getRegisteredArtifacts().contains(QueryArtifact.MODEL)) {
			...
			Collection<IAbstractArtifact> allQueries = artifactMgr
					.getArtifactsByModel(QueryArtifact.MODEL,
							includeDependencyOverride, false,
							new NullProgressMonitor());
		...
		if (artifactMgr.getRegisteredArtifacts().contains(
			...
			Collection<IAbstractArtifact> allSessions = artifactMgr
					.getArtifactsByModel(SessionFacadeArtifact.MODEL,
							includeDependencyOverride, false,
							new NullProgressMonitor());
		...
		if (artifactMgr.getRegisteredArtifacts().contains(
			...
			Collection<IAbstractArtifact> allUpdateProcedures = artifactMgr
					.getArtifactsByModel(UpdateProcedureArtifact.MODEL,
							includeDependencyOverride, false,
							new NullProgressMonitor());
		...
		if (artifactMgr.getRegisteredArtifacts().contains(
			...
			Collection<IAbstractArtifact> allAssociations = artifactMgr
					.getArtifactsByModel(AssociationArtifact.MODEL,
							includeDependencyOverride, false,
							new NullProgressMonitor());
		...
		if (artifactMgr.getRegisteredArtifacts().contains(
			...
			Collection<IAbstractArtifact> allDependencies = artifactMgr
					.getArtifactsByModel(DependencyArtifact.MODEL,
							includeDependencyOverride, false,
							new NullProgressMonitor());
		...
		if (artifactMgr.getRegisteredArtifacts().contains(
			...
			Collection<IAbstractArtifact> allAssociationClasses = artifactMgr
					.getArtifactsByModel(AssociationClassArtifact.MODEL,
							includeDependencyOverride, false,
							new NullProgressMonitor());
		...
		if (artifactMgr.getRegisteredArtifacts()
			...
			Collection<IAbstractArtifact> allPackages = artifactMgr
					.getArtifactsByModel(PackageArtifact.MODEL,
							includeDependencyOverride, false,
							new NullProgressMonitor());
	...
	public Map<String, Object> getModuleContext(ModelProject modelProject)
			throws TigerstripeException {
		ITigerstripeModelProject handle = modelProject.getProject();
		ArtifactManagerSessionImpl session = (ArtifactManagerSessionImpl) handle
				.getArtifactManagerSession();
		ArtifactManager artifactMgr = session.getArtifactManager();
		...
		Collection<IAbstractArtifact> artifacts = artifactMgr.getAllArtifacts(
				false, new NullProgressMonitor());
		context.put(MODULE_ARTIFACTS, processResults(artifacts, modelProject));

		putModuleArtifactsToContext(context, artifactMgr, modelProject);

		return context;
	}

	private void putModuleArtifactsToContext(Map<String, Object> context,
			ArtifactManager artifactMgr, ModelProject modelProject) {
		putModelSpecificArtifacts(context, MODULE_ENTITIES,
				ManagedEntityArtifact.MODEL, artifactMgr, modelProject);
		putModelSpecificArtifacts(context, MODULE_DATATYPES,
				DatatypeArtifact.MODEL, artifactMgr, modelProject);
		putModelSpecificArtifacts(context, MODULE_EVENTS, EventArtifact.MODEL,
				artifactMgr, modelProject);
		putModelSpecificArtifacts(context, MODULE_ENUMERATIONS,
				EnumArtifact.MODEL, artifactMgr, modelProject);
		putModelSpecificArtifacts(context, MODULE_EXCEPTIONS,
				ExceptionArtifact.MODEL, artifactMgr, modelProject);
		putModelSpecificArtifacts(context, MODULE_QUERIES, QueryArtifact.MODEL,
				artifactMgr, modelProject);
		putModelSpecificArtifacts(context, MODULE_SESSIONS,
				SessionFacadeArtifact.MODEL, artifactMgr, modelProject);
		putModelSpecificArtifacts(context, MODULE_UPDATEPROCEDURES,
				UpdateProcedureArtifact.MODEL, artifactMgr, modelProject);
		putModelSpecificArtifacts(context, MODULE_ASSOCIATIONS,
				AssociationArtifact.MODEL, artifactMgr, modelProject);
		putModelSpecificArtifacts(context, MODULE_DEPENDENCIES,
				DependencyArtifact.MODEL, artifactMgr, modelProject);
		putModelSpecificArtifacts(context, MODULE_ASSOCIATIONCLASSES,
				AssociationClassArtifact.MODEL, artifactMgr, modelProject);
		putModelSpecificArtifacts(context, MODULE_PACKAGES,
				PackageArtifact.MODEL, artifactMgr, modelProject);

	}

	private void putModelSpecificArtifacts(Map<String, Object> context,
			String key,
			IAbstractArtifactInternal model, ArtifactManager artifactMgr,
			ModelProject modelProject) {
		if (artifactMgr.getRegisteredArtifacts().contains(model)) {
			Collection<IAbstractArtifact> artifacts = artifactMgr
					.getArtifactsByModel(model, false,
							new NullProgressMonitor());
			artifacts = processResults(artifacts, modelProject);
			context.put(key, artifacts);
		}
	}

	private Collection<IAbstractArtifact> processResults(
			Collection<IAbstractArtifact> artifacts, ModelProject modelProject) {
		if (modelProject.isReferenced()
				&& modelProject.getReferenceLevel() == 1) {
			if (artifacts.size() > 0) {
				Collection<IAbstractArtifact> result = new ArrayList<IAbstractArtifact>(
						artifacts.size());
				for (IAbstractArtifact artifact : artifacts) {
					result.add((IAbstractArtifact) ContextProjectAwareProxy
							.newInstance(artifact,
									modelProject.getContextProject()));
				}
				return result;
			}
		}
