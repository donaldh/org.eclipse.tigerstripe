...
/**
 * An abstract implementation of a custom toolbar contribution item for
 * reflecting and changing properties of the selected objects in a workbench
 * part
 * 
 * @author melaasar
 * 
 */
public abstract class PropertyChangeContributionItem extends
		CustomContributionItem {
	 ...
	 *            The property name
	 */
	public PropertyChangeContributionItem(IWorkbenchPage workbenchPage,
	...
	/**
	 * Returns the property value of the given property id of the current
	 * operation set's
	 * 
	 * The default implementation returns the current property value of the
	 * primary object in the operation set if not empty and <code>null</code>
	 * otherwise
	 * 
	 */
	protected Object getOperationSetPropertyValue(String id) {
		List<EditPart> set = getOperationSet();
		if (!set.isEmpty()) {
	...
	/**
	 * A utility method to return the value of a given property for a given
	 * editpart
	 * 
	 * @param editPart
	 *            The editpart
	 ...
	 * @param editPart
	 *            The editpart
	 * @param useExclusiveRead
	 *            indicates if the read process willl be run in a read
	 *            execlusive operation or not
	 * @return The current value of the editpart's given property
	 */
	protected Object getPropertyValue(final IGraphicalEditPart editPart,
	...
	@Override
	protected List<EditPart> getTargetEditParts(EditPart editpart) {
		if (digIntoGroups() && editpart instanceof GroupEditPart) {
			List<EditPart> targetEPs = new ArrayList<EditPart>();
