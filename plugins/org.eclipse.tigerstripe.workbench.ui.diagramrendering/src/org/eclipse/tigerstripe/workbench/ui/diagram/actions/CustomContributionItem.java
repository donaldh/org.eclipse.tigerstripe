...
public abstract class CustomContributionItem extends AbstractContributionItem {
	...
	private List<EditPart> _operationSet = new ArrayList<EditPart>();
	...
	/**
	 * Gets a command to execute on the operation set based on the target
	 * request
	 * 
	...
	protected Command getCommand(Request request) {
		...
		Iterator<EditPart> editParts = operationSet.iterator();
	...
	protected final List<EditPart> getOperationSet() {
		if (_operationSet == null) {
			...
			if (_operationSet == null) {
				_operationSet = new ArrayList<EditPart>();
			}
		}
	...
	protected List<EditPart> createOperationSet() {
		...
		if (!selection.isEmpty()
			...
			while (selectedEPs.hasNext()) {
				...
				result.addAll(getTargetEditParts(selectedEP));
			}
		}
		return result;
	}
	 ...
	 * EditParts that matches the given condition when called on the given
	 * editpart).
	 * 
	 * @param list
	 ...
	 *         condition
	 */
	protected List<EditPart> filterEditPartsMatching(List<EditPart> list,
			EditPartViewer.Conditional condition) {
		List<EditPart> matchList = new ArrayList<EditPart>();
		Iterator<EditPart> iter = list.iterator();
		while (iter.hasNext()) {
