...
public class TabDescriptionManipulator {
	...
	public boolean isEnabled(AnnotationFilter filter) {
	...
	public void update(Annotation annotation) {
		...
		for (List<ISectionDescriptor> list : tabToSections.values()) {
			...
			for (ISectionDescriptor descriptor : list) {
				AnnotationFilter filter = (AnnotationFilter)descriptor.getFilter();
				if (filter.select(annotation))
			...
			for (ISectionDescriptor descriptor : list) {
				Boolean value = descriptor == enable ? Boolean.TRUE : Boolean.FALSE;
				classToFilter.put(descriptor.getFilter().getClass().getName(), value);
			}
		}
	}
	...
	private Map<String, List<ISectionDescriptor>> getTabToSections() {
		if (tabToSections == null) {
			...
			for (ISectionDescriptor descriptor : descriptors) {
				if (descriptor.getFilter() instanceof AnnotationFilter) {
